...
from pre_process.lstm_utils import define_columns

class SequencesCreator():

    def __init__(self, one_timestamp, ac_index, rl_index, logger):
        """constructor"""
    ...
    def vectorize(self, model_type, log, params, add_cols):
        self.log = log
        columns = define_columns(add_cols, self.one_timestamp)
    ...
    def _vectorize_seq(self, parms, columns):
        ...
        #self.logger.info(f"Unique values in x: {np.unique(vec['next_evt']['activities'])}")
        ...
        # in logs with more than one timestamp: no need for waiting time (we are intrested in duration time)
        #if not parms['one_timestamp']:       
            #vec['next_evt']['times'] = vec['next_evt']['times'][:, 0][:, np.newaxis]
        return vec
    ...
    def reformat_events(self, columns, one_timestamp):
        ...
        temp_data = list()
        case_id_lst = list()
        prefix_length_lst = list()
        log_df = self.log.to_dict('records')
        sort_key = 'end_timestamp' if one_timestamp else 'start_timestamp'
        log_df = sorted(log_df, key=lambda x: x['caseid'])        
        #log_df = sorted(log_df, key=lambda x: (x['caseid'], sort_key))
        for case, group in itertools.groupby(log_df, key=lambda x: x['caseid']):
            trace_events = list(group)
            trace = sorted(trace_events, key=itemgetter(sort_key))
            ...
            temp_dict = {**{'caseid': case}, **temp_dict}
            temp_data.append(temp_dict)
            trace_length = len(trace)
            trace_id = trace[0].get('caseid')
            case_id_lst.extend([trace_id] * (trace_length+1))
            prefix_length_lst.extend(list(range(1, trace_length + 2)))
        self.case_id_lst= case_id_lst
        self.prefix_length_lst = prefix_length_lst
        return temp_data
    
    def get_inference_lists(self):
        return (self.case_id_lst, self.prefix_length_lst)