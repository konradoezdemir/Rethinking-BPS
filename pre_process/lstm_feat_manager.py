...
class FeaturesMannager():
    ...
    def add_calculated_times(self, log):
        ...
        for _, group in itertools.groupby(log, key=lambda x: x['caseid']):
            ...
            for i in range(0, len(events)):
                ...
                # Achtung! change the original implementation to handle error caused by start timestamp!
                if self.one_timestamp:
    ...
    def _scale_inter(self, log, add_cols):
        ...
        for col in add_cols:
            ...
            elif (col == 'weekday' or col == 'train' or col == 'val' or 
                  col == 'test'):
                continue
            else:
...
def scale_values(log, params):
    # Features treatment
    inp = FeaturesMannager(params)
    # Register scaler
    inp.register_scaler(params['model_type'], params['scaler'])        
    # Scale features
    log, params['scale_args'] = inp.calculate(log, [])
    return log, params  